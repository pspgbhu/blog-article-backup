...
我在本地 Chrome ( v63.0.3239.132 ) 上测试 JS 阻塞 DOM 解析特性的时候还发现了其他一些有趣的现象，下面我们就一起来看一下。

### script 标签位置产生的不同影响

首先，script 标签最好永远不要放在任何 DOM 的前面，因为 JS 会阻塞 DOM 的解析。那么除此之外 script 标签的位置还会造成什么其他影响呢？

#### CSS 阻塞特性的消失

当 script 引入一个**外部脚本**，并且有**一个内联或外部样式在该外部脚本之后定义或引入**，则该 CSS 样式将会丧失对页面的阻塞作用。即：如果满足以上条件，并且该样式在 DOM 之后定义，则 DOM 就有可能在该 CSS 规则生效前显示在页面上。

遗憾的是，目前我只观察到了这种现象，但并不太理解浏览器这么设计的目的是什么。
